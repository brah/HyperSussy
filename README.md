...
- **Passive whale discovery** -- no manual seed list required; high-volume addresses are promoted to a tracked list from the trade stream
...
| **Whale Tracker** | Whale discovery, large positions (>5% of coin OI), significant position changes (>$1M) |
| **TWAP Detector** | Evenly-spaced fill patterns indicative of algorithmic TWAP execution |
...
Orchestrator
  ...
  |                 |
  |                 +-> Sinks (structured log, Discord/Telegram stubs)
  |
...
| `HYPERSUSSY_WHALE_VOLUME_THRESHOLD_USD` | `500000` | Volume threshold for whale promotion |
...
src/hypersussy/
    ...
    engines/
        ...
        whale_tracker.py       # Discover whales, poll positions, detect changes
        twap_detector.py       # Detect evenly-spaced fill patterns
...
tests/                         # 75 tests covering all engines, storage, parsers, etc.
```
