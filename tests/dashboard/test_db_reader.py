...
def db_path(tmp_path: pytest.TempPathFactory) -> str:
    ...
    conn.executemany(
        """INSERT INTO asset_snapshots
           ...
           VALUES (?,?,?,?,?,?,?,?,?)""",
        [
            ("BTC", now_ms - 2 * hour_ms, 500, 25_000_000, 50_000, 49_950, 0.0001, 0.001, 1e9),
            ("BTC", now_ms - hour_ms,     510, 25_500_000, 50_100, 50_050, 0.0002, 0.001, 1.1e9),
            ("ETH", now_ms - hour_ms,     1000, 3_000_000, 3_000, 2_990, -0.0001, -0.001, 5e8),
        ],
    )
    ...
    conn.executemany(
        """INSERT INTO alerts
           ...
           VALUES (?,?,?,?,?,?,?)""",
        [
            ("a1", "funding_anomaly", "high",   "BTC", "Funding spike", "...", now_ms - 2000),
            ("a2", "whale_position",  "medium", "ETH", "Whale detected", "...", now_ms - 1000),
            ("a3", "funding_anomaly", "low",    "ETH", "Minor anomaly",  "...", now_ms),
        ],
    )
    conn.commit()
    conn.close()
